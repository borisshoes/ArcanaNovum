...
public class ContinuumAnchor extends ArcanaBlock {
   ...
   public ContinuumAnchor(){
      ...
      item = new ContinuumAnchorItem(block,addArcanaItemComponents(new Item.Settings().maxCount(1).fireproof()));
   ...
   public List<List<Text>> getBookLore(){
      ...
      list.add(List.of(TextUtils.withColor(Text.literal(" Continuum Anchor").formatted(Formatting.BOLD), ArcanaColors.BETTER_DARK_BLUE),Text.literal("\nWhen fed with Exotic Matter, the Anchor chunk loads a 5x5 chunk area with Mob Spawning and Random Ticks. It is able to stimulate mobs such that they do not despawn when a player is not present.\nThe Anchor also ").formatted(Formatting.BLACK)));
      list.add(List.of(TextUtils.withColor(Text.literal(" Continuum Anchor").formatted(Formatting.BOLD), ArcanaColors.BETTER_DARK_BLUE),Text.literal("\nproduces lazy and border chunks beyond the 5x5 area.\n\nThe Anchor can be turned off with a redstone signal and its fuel can be removed by an empty hand. Additional fuel may also be added while still in use.").formatted(Formatting.BLACK)));
      return list;
   }
