            ...
            literal("arcana").then(literal("gamerule").requires(source -> source.hasPermissionLevel(4))
                  .executes(ctx -> {
                     values.stream().filter(v -> v.command != null).forEach(value ->
                           ctx.getSource().sendFeedback(MutableText.of(new TranslatableTextContent(value.command.getterText, null, (Object[]) value.value)), false));
                     return 1;
                  }));
      values.stream().filter(v -> v.command != null).forEach(value ->
            out.then(literal("gamerule").then(literal(value.name)
                  ...
                  .then(argument(value.name, value.getArgumentType()).suggests(value.suggestions)
                        .executes(ctx -> {
                           ...
                           ((CommandContext<ServerCommandSource>) ctx).getSource().sendFeedback(MutableText.of(new TranslatableTextContent(value.command.setterText, null, (Object[]) value.value)), true);
                           this.save();
                           return 1;
                        })))));
      return out;
